...
const seed = [
  {username:'mahmoud', name:'محمود ابوقاعود', role:'owner', password:'owner2025'},
  {username:'khaled', name:'خالد', role:'teacher', password:'teacher2025'},
...
export default function Login({ onLogged }){
  ...
  useEffect(() => {
    (async () => {
      ...
      if (snap.empty) {
        for (const u of seed) {
          await setDoc(doc(col, u.username), u);
        }
      }
    })();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-lux flex items-center justify-center p-6">
      <div className="card p-6 w-full max-w-md">
        <div className="flex items-center justify-center gap-2 text-royal">
          <span className="text-2xl">gift-tmfv</span><span className="text-gold text-2xl">🎁</span>
        </div>
        ...
        <p className="mt-4 text-center footer">تم تصميم الموقع من قبل محمود ابوقاعود 💎</p>
      </div>
    </div>
  );
}